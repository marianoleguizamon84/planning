...
$user = getUserName();
$is_admin = (authGetUserLevel($user) == 3);
...
if ($is_admin || isEncargadoDeAreaOAula($row['room_id']) || $create_by == $user) {
    $puedeEditar = true;
}
...
if ($provisional_enabled && ($status == STATUS_PROVISIONAL) && $puedeEditar)
{
...
<tbody>
  ...
  if ($provisional_enabled)
  {
    ...
    echo "<td>" . (($status == STATUS_PROVISIONAL) ? get_vocab("provisional") : get_vocab("confirmed"));
    echo "</td>\n";
    echo "</tr>\n";
  }
  ?>
